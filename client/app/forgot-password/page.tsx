...
export default function ForgotPasswordPage() {
  ...
  const onSubmit = async (data: ForgotPasswordInput) => {
    try {
      setServerError("");
      // Using the endpoint configured in your backend
      await api.post(AUTH_ROUTES.FORGET_PASSWORD, data);
      setIsSent(true);
    } catch (err) {
      // Explicitly typing the error to avoid "any"
      ...
      <div className="max-w-md w-full bg-bg-card p-8 rounded-2xl shadow-md border border-border-main">
        ...
        {!isSent ? (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            {serverError && (
              <p className="text-xs text-danger font-semibold bg-danger/10 p-2 rounded border border-danger/20">
                {serverError}
              </p>
            )}
          ...
          <div className="text-center bg-primary/5 p-6 rounded-xl border border-primary/10">
            ...
            >
              Did no    t receive an email? Try again
            </button>
          </div>
        )}
      </div>
